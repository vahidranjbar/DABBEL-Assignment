...
from utils import create_dfs, generate_response_dataframe
...
def get_result(sensor):
  # For the sake of performance, instead of working with dataframe,
  #  all recoreds associated with each Sensor will be converted into dictionary objects
  # NOte: Complexity: O(N)
  ...
  # Sort the combined list of both sensor_prediction_list and sensor_reading_list with respect to time_stamp
  ...
  for i in range(0, len(temp_list_sorted)-1):
    ...
    # If current_record_flag and next_record_flag are the same it means both records are from the same table
    # and not required to calculate the distance between them, otherwise calculate the distance and add a new 
    # tuple into the distance list
...
if __name__=="__main__":
  ...
  """
  # Create two main input tables
  reading_df, prediction_df=create_dfs() 
  #Note: 
      # 'R': Reading table
      # 'P': Predcition table
  reading_df["flag"]='R'
  ...
  # To store the final response
  store_list=[]
  t1=time.time()    
  for sensor in sensor_list:
    result=get_result(sensor)
    store_list.extend(result)    
  print(time.time()-t1)     
  response_dataframe=generate_response_dataframe(store_list, "main")
